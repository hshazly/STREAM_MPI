...
main()
    ...
    MPI_Win wina, winb, winc;
    
	// MODIFICATION: Allocate the array using MPI_Win_allocate
	// NOTE that the OFFSET parameter is not used in this version of the code!
    array_bytes = array_elements * sizeof(STREAM_TYPE);
    
    int key = 0;

    //mpi_ret = MPI_Win_allocate_shared(array_bytes, sizeof(double), MPI_INFO_NULL, newcomm, (void **)&a, &wina);
    mpi_ret = MPI_Win_allocate(array_bytes, sizeof(double), MPI_INFO_NULL, MPI_COMM_WORLD, (void **)&a, &wina);
    ...
    mpi_ret = MPI_Win_allocate(array_bytes, sizeof(double), MPI_INFO_NULL, MPI_COMM_WORLD, (void **)&b, &winb);
    ...
    mpi_ret = MPI_Win_allocate(array_bytes, sizeof(double), MPI_INFO_NULL, MPI_COMM_WORLD, (void **)&c, &winc);
...
#pragma omp parallel for
	...
	if (myrank == 0) {
                float maxtimes[] = {0, 0, 0, 0};
                float mintimes[] = {0, 0, 0, 0};
			...
			for (j=0; j<4; j++) {
				...
				for (i=0; i<numranks; i++) {
					...
					tmin = MIN(tmin, TimesByRank[4*NTIMES*i+j*NTIMES+k]);
                                        tmax = MAX(tmax, TimesByRank[4*NTIMES*i+j*NTIMES+k]);
			        }
				maxtimes[j] += tmax;
                                mintimes[j] += tmin;
                                times[j][k] = tmin;
